...
export function structuredMetaPrompt(
  ...
  return [
    ...
    '  "imageSuggestions": [{"section": string, "kind": "image"|"graphic"|"graph", "alt": string, "caption": string, "prompt": string}]',
    "      // 2-4 figures that would help this article. section = an EXACT source heading.",
    "      // kind = MOST SUITABLE format: 'image' (a photorealistic PHOTO — PREFER THIS),",
    "      //   'graphic' (a polished, photorealistic graphic), or 'graph' (a real DATA CHART —",
    "      //   ONLY when the section actually has comparable numbers).",
    "      // alt = concrete machine-readable alt text. caption = a short visible caption; put the",
    "      //   detailed part labels HERE (not inside the image — the renderer garbles small text).",
    "      // prompt = a vivid, detailed, PHOTOREALISTIC / AI-realistic generation prompt. Demand a",
    "      //   real photograph-quality look with true materials, lighting and depth — NEVER flat",
    "      //   clipart, cartoon, line-art, or a generic white/grey studio cobot. If the figure shows",
    "      //   a robotic arm, describe a TROSSEN arm: a matte-black 3D-printed carbon-fiber research",
    "      //   arm with triangular truss/lattice cut-outs, black servo joints, a black parallel",
    "      //   gripper on a linear rail on a maple workbench, with the white 'TROSSEN' wordmark on",
    "      //   the side. Do NOT request callout/label text inside the image; keep any words to a few",
    "      //   short, correctly-spelled ones.",
