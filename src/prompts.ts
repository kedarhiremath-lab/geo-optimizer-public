...
export function structuredMetaPrompt(
  ...
  return [
    ...
    '  "imageSuggestions": [{"section": string, "alt": string, "caption": string, "prompt": string}]',
    "      // 2-4 figures that would help this article. section = an EXACT section",
    "      // heading from the source. alt = concrete machine-readable alt text",
    "      // (what the figure literally shows). caption = a short visible caption.",
    "      // prompt = a ready-to-use image-generation prompt for a TEXT-FREE illustration",
    "      //   (describe the visual scene only — NO labels, callouts, words, or diagram",
    "      //   annotations; image models misspell text. Put any labels in the alt instead).",
    "      // Tie each to a real section and its content; never decorative stock-photo ideas.",
    "}",
    "",
    targetsBlock(config),
    "",
