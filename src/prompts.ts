...
export function structuredMetaPrompt(
  ...
  return [
    ...
    "      // kind = the MOST SUITABLE format for that section: 'image' (photorealistic",
    "      //   photo / 3D render), 'graphic' (polished infographic/diagram), or 'graph'",
    "      //   (a data chart — only when the section actually has comparable numbers).",
    "      // alt = concrete machine-readable alt text. caption = a short visible caption.",
    "      // prompt = a vivid, detailed, AI-forward image-generation prompt matching the kind",
    "      //   (premium, high production value — never flat clipart or plain cartoon). A little",
    "      //   legible text is fine; keep it minimal and put full labels in the alt.",
